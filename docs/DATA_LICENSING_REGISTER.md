...
The platform displays **no licensed commercial data**. Anomaly and price values are manual indicative samples referencing public sources, labelled as such. The Flow Map and the homepage globe read one hull from TankerMap's documented public API (informational use, attributed, delayed). Homepage sea lanes are desk geometry, not AIS. No scraping. No paid-publication content reproduced. No real-time exchange data.
...
| Single-hull AIS (watched vessel) | TankerMap public API | No formal open-data licence. Informational use of documented public endpoints (`/api/search`, `/api/vessels/{id}`, `/api/vessels/{id}/track`). | ✅ Cited, delayed, one hull | Flow Map `?vessel=` and the homepage globe marker. Same hull. Attribution to TankerMap. Not a navigation, sanctions or compliance source. Desk note on the same hull is ours and labelled indicative. Homepage lanes are schematic geometry. |
